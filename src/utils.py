...
''' def get_atom_features(smile_list):
    ...
    for atom, aromatic, bonds, hydrogen, hybrid in zip(atomic_number, aromaticity, num_bonds, bonded_hydrogens, hybridization):
        ...
        key_features_combined = f"{atom}_{aromatic}_{bonds}_{hydrogen}_{hybrid}"
        ...
        feature = np.concatenate((atomic_feature, aromatic_feature, bonds_feature, hydrogen_feature, hybrid_feature))
        
        features_dict[key_features_combined] = feature
...
''' def get_atom_features(smile_list):
    ...
    features_combined = [f"{atomic}_{aromatic}_{bonds}_{hydrogen}_{hybrid}" 
                        for atomic, aromatic, bonds, hydrogen, hybrid 
                        in zip(atomic_number, aromaticity, num_bonds, bonded_hydrogens, hybridization)]
    features_set = list(set(features_combined))
    
    codificador = OneHotEncoder()
    codificador.fit(np.array(features_set).reshape(-1,1))
    
    features_dict = defaultdict(list)
    
    for atom, aromatic, bonds, hydrogen, hybrid in zip(atomic_number, aromaticity, num_bonds, bonded_hydrogens, hybridization):
        
        clave = f"{atom}_{aromatic}_{bonds}_{hydrogen}_{hybrid}"
        feature = codificador.transform([[clave]]).toarray()[0]
        features_dict[clave] = feature
